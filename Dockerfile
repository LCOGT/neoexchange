...
# The contents will be discarded from the final image.
RUN curl -fsSL --insecure https://astro.troja.mff.cuni.cz/projects/damit/files/${DAMIT_VERSION}.tar.gz | tar xzf - \
        && cd ${DAMIT_VERSION} \
        && cd convexinv \
        && make \
        && cd ../conjgradinv \
...
RUN pip3 --no-cache-dir install --upgrade pip \
    && python3 -m pip --no-cache-dir install --upgrade numpy==1.23.5 wheel \
