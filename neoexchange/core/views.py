...
def run_swarp_make_reference(ref_dir, configs_dir, dest_dir, ref_files=None, match='*.fits'):
    ...
    If [ref_files] is None, then a search in <ref_dir> using [match] is done to
    find frames; if [ref_files] is not None, it should be a list of filepaths
    to make into the reference frame.
...
def create_latex_table(body_or_name, return_table=False, deluxetable=False, dbg=False):
    ...
    for field in cleaned_data.get('field_list', []):
        for obs_record in blocks.order_by('block_start'):
            ...
            frames = Frame.objects.filter(block=obs_record,frametype__in=(Frame.SPECTRUM_FRAMETYPE,Frame.BANZAI_QL_FRAMETYPE,Frame.BANZAI_RED_FRAMETYPE,Frame.SINGLE_FRAMETYPE,Frame.MRO_RED_FRAMETYPE))
            if field == 'obstype':
