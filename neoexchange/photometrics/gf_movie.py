...
def make_gif(frames, title=None, sort=True, fr=333):
    ...
    anim = FuncAnimation(fig, update, frames=fits_files, blit=False, interval=fr)  # takes in fig, update function, and frame rate set to 3fps
...
def update(fits_file):
    ...
    # finish up plot
    ax.set_title('UT Date: {}'.format(date.strftime('%x %X')), pad=10)
