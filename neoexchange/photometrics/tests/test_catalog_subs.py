...
class TestAllOfPipelineAstrometry(TestCase):

    def setUp(self):
        from django.core.management import call_command
        self.obs_date = '20200110'
        self.dataroot = '/home/jchatelain/Asteroids'

        # Create test body
        params = {  'provisional_name': 'P10VVYB',
                    ...
                    'fast_moving': True,
                    'urgency': None,
                    'epochofel': datetime(2020, 1, 17, 0, 0),
                    'orbit_rms': 0.78,
                    'orbinc': 5.72685,
                    'longascnode': 307.4646,
                    'argofperih': 264.45149,
                    'eccentricity': 0.6107423,
                    'meandist': 1.4322771,
                    'meananom': 332.40845,
                    'perihdist': None,
                    'epochofperih': None,
                    ...
                    'slope': 0.15,
                    'score': 77,
                    'discovery_date': datetime(2020, 1, 1, 12, 0),
                    'num_obs': 35,
                    'arc_length': 6.0,
                    'not_seen': 9.796749799155092,
                    'updated': True,
                    'ingest': datetime(2020, 1, 1, 18, 50, 8),
                    'update_time': datetime(2020, 1, 16, 19, 7, 19)
                    }
        self.body_with_provname, created = Body.objects.get_or_create(**params)

        # Create test proposal
        neo_proposal_params = { 'code'  : 'LCO2020A-011',
                                'title' : 'LCOGT NEO Follow-up Network'
                              }
        self.neo_proposal, created = Proposal.objects.get_or_create(**neo_proposal_params)

        # Create test sblock
        sblock_params = {'body'     : self.body_with_provname,
                         'proposal' : self.neo_proposal,
                         'cadence': False,
                         'rapid_response': True,
                         'calibsource': None,
                         'block_start': datetime(2020, 1, 11, 1, 20),
                         'block_end': datetime(2020, 1, 11, 12, 40),
                         'groupid': '2020 AO_V37-20200111',
                         'tracking_number': '919341',
                         'period': None,
                         'jitter': None,
                         'timeused': 1514.0,
                         'active': True
        ...
        # Create test block
        block_params = { 'body'     : self.body_with_provname,
                         'superblock' : self.test_sblock,
                         'telclass': '1m0',
                         'site': 'elp',
                         'calibsource': None,
                         ...
                         'block_start': datetime(2020, 1, 11, 1, 20),
                         'block_end': datetime(2020, 1, 11, 12, 40),
                         'request_number': '2031992',
                         'num_exposures': 38,
                         'exp_length': 8.0,
                         'num_observed': 1,
                         'when_observed': datetime(2020, 1, 11, 7, 1, 39),
                         'active': True,
                         'reported': False,
                         'when_reported': None
                       }
        ...
        call_command('update_blocks')

    def test_from_download_process_data(self):
        from django.core.management import call_command
        call_command('download_process_data', '--date', self.obs_date, '--datadir', self.dataroot, '--skip-download')
        frames = Frame.objects.filter(block=self.test_block)
        for frame in frames:
            print("{} zeropoint:{}+/-{}, FWHM:{}".format(frame.filename, frame.zeropoint, frame.zeropoint_err, frame.fwhm))
            sources = CatalogSources.objects.filter(frame=frame)
            for source in sources:
                print("----RA:{} Dec:{} mag:{}+/-{} (Peak: {} / Bkgnd: {} / Threshold: {})".format(source.obs_ra, source.obs_dec, source.obs_mag, source.err_obs_mag, source.flux_max, source.background, source.threshold))
